...
function initMobileNav(){
  ...
  nav.querySelectorAll('a,button').forEach(function(el){
    if(el.hasAttribute('data-dropdown-toggle')) return;
...
function initHeaderShadow(){
  ...
  if(!header) return;
  function update(){ header.classList.toggle('is-scrolled', window.scrollY > 10); }
...
function initHeroSlider(){
  ...
  function go(n){
    ...
    dots.querySelectorAll('button').forEach(function(b,i){ b.classList.toggle('is-active', i===idx); });
  }
  function start(){ clearInterval(timer); timer = setInterval(function(){ go(idx+1); }, 6000); }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  initFaq();
  initCounters();
  wireForm('infoForm','infoFormWrap','info_thanks_title','info_thanks_body');
